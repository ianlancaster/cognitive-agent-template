...
**Gate — per contribution class.** Contributions are classified, and each class has its own mode. Classify each candidate by path, then check `scripts/role-template.sh should-contribute .template-sync.json <class>` per class (exit 0 = that class may contribute). Base classes: **`cognitive-authority`** = `memory/feedback_*` (rules), `memory/cognition/beliefs.md` (belief statements), `scripts/*` (executable) — never auto; **`cognitive-knowledge`** = `memory/{gotcha_,domain_,reference_}*`, `memory/cognition/{insight-log,ideation}.md`, `knowledge/*`. A flavored base may define further classes (its docs say how its paths map). If every class gates closed or there are no candidates, this phase is a **no-op** — report "no contribution (reason)" and move on. A zero-yield session must cost nothing.

1. **Find candidates.** `scripts/role-template.sh candidates <lastContributedCommit>` lists portable-candidate files changed since your last contribution (role-general memory, belief statements, insight log, knowledge, non-base scripts — plus any paths a flavored base added via `.template-candidates`). Instance-only files are never listed. No candidates → no-op.
...
4. **Apply per `contributionMode`:**
   ...
   - **`auto`** — clone the role template, stage the distilled artifacts on a **contribution branch**, run any gates the template ships against that branch (re-run the leak-check at minimum; a template-shipped contribution gate — e.g. a flavored base's `scripts/*-gate.sh` — must pass where it can run), then merge to the default branch and push. **Auto never lands what a gate rejected or could not run:** on gate failure or an unrunnable required gate, push the branch unmerged as a quarantined contribution and flag it for the operator — auto's authority is to add clean content, never to bypass a gate. (A branch also survives a push race or a hosted-remote protection rule, where a direct default-branch push would fail.) **Never overwrite:** if a distilled artifact conflicts with one already there (same rule/belief, different content), quarantine yours as a competing artifact attributed to this instance and flag it — auto adds and flags, it never resolves a conflict. A graduated belief matching one already present **increments its "converged across K instances" annotation** instead of duplicating it.
